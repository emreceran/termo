...
class WebsiteSaleForm(WebsiteForm):
    ...
    @http.route('/website/form/usp.sale.order', type='http', auth="public", methods=['POST'], website=True)
...
class WebsiteSale(http.Controller):
    _express_checkout_route = '/usp/express_checkout'
    ...
    def sitemap_shop(env, rule, qs):
        if not qs or qs.lower() in '/usp':
            yield {'loc': '/usp'}

        Category = env['product.public.category']
        dom = sitemap_qs2dom(qs, '/usp/category', Category._rec_name)
        dom += env['website'].get_current_website().website_domain()
        for cat in Category.search(dom):
            loc = '/usp/category/%s' % slug(cat)
    ...
    @http.route([
        '/usp',
        '/usp/page/<int:page>',
        '/usp/category/<model("product.public.category"):category>',
        '/usp/category/<model("product.public.category"):category>/page/<int:page>',
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        keep = QueryURL('/usp', **self._shop_get_query_url_kwargs(category and int(category), search, min_price, max_price, **post))
        ...
        if category:
            url = "/usp/category/%s" % slug(category)
        ...
        return request.render("termo.products", values)

    @http.route(['/usp/<model("product.template"):product>'], type='http', auth="public", website=True, sitemap=True)
    def product(self, product, category='', search='', **kwargs):
        return request.render("website_sale.product", self._prepare_product_values(product, category, search, **kwargs))

    @http.route(['/usp/product/<model("product.template"):product>'], type='http', auth="public", website=True, sitemap=False)
    def old_product(self, product, category='', search='', **kwargs):
        # Compatibility pre-v14
        return request.redirect(_build_url_w_params("/usp/%s" % slug(product), request.params), code=301)

    @http.route(['/usp/product/extra-images'], type='json', auth='user', website=True)
    ...
    @http.route(['/usp/product/clear-images'], type='json', auth='user', website=True)
    ...
    @http.route(['/usp/product/remove-image'], type='json', auth='user', website=True)
    ...
    @http.route(['/usp/product/resequence-image'], type='json', auth='user', website=True)
    ...
    @http.route(['/usp/product/is_add_to_cart_allowed'], type='json', auth="public", website=True)
    ...
    @http.route(['/usp/change_pricelist/<model("product.pricelist"):pricelist>'], type='http', auth="public", website=True, sitemap=False)
    def pricelist_change(self, pricelist, **post):
        ...
        return request.redirect(redirect_url or '/usp')

    @http.route(['/usp/pricelist'], type='http', auth="public", website=True, sitemap=False)
    def pricelist(self, promo, **post):
        redirect = post.get('r', '/usp/cart')
    ...
    @http.route(['/usp/cart'], type='http', auth="public", website=True, sitemap=False)
    def cart(self, access_token=None, revive='', **post):
        ...
        if access_token:
            ...
            if abandoned_order.state != 'draft':  # abandoned cart already finished
                ...
                return request.redirect('/usp/cart')
            elif revive == 'merge':
    ...
    @http.route(['/usp/cart/update'], type='http', auth="public", methods=['POST'], website=True)
    def cart_update(
        ...
        if express:
            return request.redirect("/usp/checkout?express=1")

        return request.redirect("/usp/cart")

    @http.route(['/usp/cart/update_json'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    ...
    @http.route('/usp/save_shop_layout_mode', type='json', auth='public', website=True)
    def save_shop_layout_mode(self, layout_mode):
        assert layout_mode in ('grid', 'list'), "Invalid shop layout mode"
        request.session['website_sale_shop_layout_mode'] = layout_mode

    @http.route(['/usp/cart/quantity'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    ...
    @http.route(['/usp/cart/clear'], type='json', auth="public", website=True)
    def clear_cart(self):
        order = request.website.sale_get_order()
    ...
    def checkout_check_address(self, order):
        ...
        if not all(order.partner_id.read(billing_fields_required)[0].values()):
            return request.redirect('/usp/address?partner_id=%d' % order.partner_id.id)
        ...
        if not all(order.partner_shipping_id.read(shipping_fields_required)[0].values()):
            return request.redirect('/usp/address?partner_id=%d' % order.partner_shipping_id.id)
    ...
    def checkout_redirection(self, order):
        ...
        if not order or order.state != 'draft':
            ...
            return request.redirect('/usp')

        if order and not order.order_line:
            return request.redirect('/usp/cart')

        if request.website.is_public_user() and request.website.account_on_checkout == 'mandatory':
            return request.redirect('/web/login?redirect=/usp/checkout')

        # if transaction pending / done: redirect to confirmation
        tx = request.env.context.get('website_sale_transaction')
        if tx and tx.state != 'draft':
            return request.redirect('/usp/payment/confirmation/%s' % order.id)
    ...
    @http.route(['/usp/address'], type='http', methods=['GET', 'POST'], auth="public", website=True, sitemap=False)
    def address(self, **kw):
        ...
        # IF ORDER LINKED TO A PARTNER
            ...
            if partner_id > 0:
                ...
                return request.redirect('/usp/checkout')
        ...
        if 'submitted' in kw and request.httprequest.method == "POST":
            ...
            if errors:
                ...
                if mode[1] == 'billing':
                    ...
                    if not kw.get('use_same'):
                        kw['callback'] = kw.get('callback') or \
                            (not order.only_services and (mode[0] == 'edit' and '/usp/checkout' or '/usp/address'))
                    ...
                    if kw.get('callback', False) != '/usp/confirm_order':
                        request.website.sale_get_order(update_pricelist=True)
                ...
                if not errors:
                    return request.redirect(kw.get('callback') or '/usp/confirm_order')
    ...
    @http.route(['/usp/checkout'], type='http', auth="public", website=True, sitemap=False)
    def checkout(self, **post):
        ...
        if order.partner_id.id == request.website.user_id.sudo().partner_id.id:
            return request.redirect('/usp/address')
        ...
        if post.get('express'):
            return request.redirect('/usp/confirm_order')
    ...
    @http.route(['/usp/confirm_order'], type='http', auth="public", website=True, sitemap=False)
    def confirm_order(self, **post):
        ...
        if extra_step.active:
            return request.redirect("/usp/extra_info")

        return request.redirect("/usp/payment")

    # ------------------------------------------------------
    # Extra step
    # ------------------------------------------------------
    @http.route(['/usp/extra_info'], type='http', auth="public", website=True, sitemap=False)
    ...
    def _get_express_shop_payment_values(self, order, **kwargs):
        ...
        return {
            ...
            'transaction_route': f'/usp/payment/transaction/{order.id}',
            'express_checkout_route': self._express_checkout_route,
            'landing_route': '/usp/payment/validate',
        }
    ...
    @http.route('/usp/payment', type='http', auth='public', website=True, sitemap=False)
    ...
    @http.route('/usp/payment/get_status/<int:sale_order_id>', type='json', auth="public", website=True)
    ...
    @http.route('/usp/payment/validate', type='http', auth="public", website=True, sitemap=False)
    def shop_payment_validate(self, sale_order_id=None, **post):
        ...
        return request.redirect('/usp/confirmation')

    @http.route(['/usp/confirmation'], type='http', auth="public", website=True, sitemap=False)
    ...
    @http.route(['/usp/print'], type='http', auth="public", website=True, sitemap=False)
    ...
    @http.route(['/usp/config/product'], type='json', auth='user')
    ...
    @http.route(['/usp/config/attribute'], type='json', auth='user')
    ...
    @http.route(['/usp/config/website'], type='json', auth='user')
    ...
    @http.route(['/usp/country_infos/<model("res.country"):country>'], type='json', auth="public", methods=['POST'], website=True)
    ...
    # --------------------------------------------------------------------------
    @http.route('/usp/products/recently_viewed_update', type='json', auth='public', website=True)
    ...
    @http.route('/usp/products/recently_viewed_delete', type='json', auth='public', website=True)
...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    @http.route(
        '/usp/payment/transaction/<int:order_id>', type='json', auth='public', website=True
    )
